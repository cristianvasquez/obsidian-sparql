 ...
 */
export async function renderSparqlView(source, container, context, debug = false) {
  try {
    // Get active file
    ...
    if (parsed.queryType === 'SELECT') {
      ...
      await renderSelectResults(results, container, context, debug, replacedQuery)
    } else if (parsed.queryType === 'CONSTRUCT') {
      results = await context.triplestore.construct(replacedQuery)
      await renderConstructQuery(results, container, context, debug, replacedQuery)
    } else {
 ...
 */
async function renderConstructQuery(results, container, context, debug, query) {
  let markdown = ''
  ...
  if (!results || results.length === 0) {
    ...
    const turtleContent = await resultsToMarkdownTurtle(results, context.app, '')
